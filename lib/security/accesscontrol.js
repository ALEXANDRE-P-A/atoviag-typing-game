      ...
      /* ----- check if password matches (starts here) ----- */
        if(!await bcrypt.compare(password, results[0].password)){
          ...
          if(results[0].locked && 
            ...
            return done(null, false, req.flash("message", `Account is locked. Please try again after ${moment(results[0].locked).add(ACCOUNT_LOCK_TIME + 540, "minutes").format("MMMM Do YYYY, hh:mm:ss")}`));
          }
          ...
          /* ----- count account success login history (starts here) ----- */
          let cnt = await MySQLClient.executeQuery(
            await sql("COUNT_LOGIN_HISTORY"),
            [user.id, 1]
          );
          console.log(`success histories : ${cnt[0].count}`);
          if(cnt[0].count > 5)
            console.log("success login histories are more than 5 times");
          /* ----- count account success login history (ends here) ----- */
