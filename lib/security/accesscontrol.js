    ...
    if(results.length === 0) // if does not exist, message
      ...
      /* ----- check if password matches (starts here) ----- */
        if(!await bcrypt.compare(password, results[0].password)){
          ...
          let cnt = await MySQLClient.executeQuery(
            await sql("COUNT_LOGIN_HISTORY"),
            [user.id, LOGIN_STATUS.SUCCESS]
          );

          if(cnt[0].count > MAX_LOGIN_HISTORY){
            console.log(`TYPE OF COUNT : ${typeof(cnt[0].count)}`);
            console.log(`MAX_LOGIN_HISTORY TYPE : ${typeof(MAX_LOGIN_HISTORY)}`);
            console.log(`COUNT : ${cnt[0].count}`);
            console.log(`MAX_LOGIN_HISTORY : ${MAX_LOGIN_HISTORY}`);
          }
